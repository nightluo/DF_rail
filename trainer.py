...
def WGAN_trainer(opt):
    ...
    generator.load_state_dict(torch.load('./pretrained_model/track_v4_deepfillv2_WGAN_G_epoch40_batchsize4.pth'))
    discriminator = utils.create_discriminator(opt)
    discriminator.load_state_dict(torch.load('./pretrained_model/track_v4_deepfillv2_WGAN_D_epoch40_batchsize4.pth'))
    ...
    for epoch in range(opt.resume_epoch, opt.epochs):
        for batch_idx, (img, height, width) in enumerate(dataloader):
            ...
            for i in range(opt.batch_size):
                mask[i] =  mask[i] = torch.from_numpy(train_dataset.InpaintDataset.random_ff_mask(shape=(height[0], width[0])).astype(np.float32)).cuda()
